...
public class AzureTableStorageLogger
{
    ...
    {
        var location = BuildLocation(callerFilePath, callerMemberName, callerLineNumber);
        await LogAsync(
            LogLevel.Debug,
            message,
            traceId,
    ...
    /// <summary>
    /// Builds a location string from caller information.
    /// </summary>
    ...
    /// <returns>A formatted location string (e.g., "Program.cs:Main:42").</returns>
        ...
        int lineNumber)
    {
        ...
        var method = string.IsNullOrEmpty(memberName) ? "Unknown" : memberName;
        return $"{fileName}:{method}:{lineNumber}";
    }
